...
@Component({
  selector: 'app-pv-generator',
  imports: [FormsModule, CommonModule],
  templateUrl: './pv-generator.component.html',
  styleUrl: './pv-generator.component.css'
})
export class PvGeneratorComponent {
  ...
  generer_pv() {
    this.zone.run(() => {
      ...
      fetch(`https://${this.backend_endpoint}/ask_to_chat_gpt`, {
        method: 'POST',
        body: JSON.stringify({
          input: `Génère un PV formel à partir de ce texte : "${this.text_brut}"`
        }),
        mode: 'cors'
      })
      .then(response => response.json())
      .then(data => {
        this.resultat.emit(this.extract_pv_from_gpt_answer(data.message));
  ...
  extract_pv_from_gpt_answer(gpt_answer: string) {
